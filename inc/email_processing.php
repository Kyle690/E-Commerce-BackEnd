 ...
 * Time: 12:37 PM
 */
class email_processing
{

    function order_update($order_number, $process, $db){



        $sql_shop_details = "SELECT * FROM store_details WHERE id =0";
        $store_details = $db->select($sql_shop_details);
        $shop_contact = $store_details[0]['contact_num'];
        $shop_address = $store_details[0]['address'];
        $shop_street = $store_details[0]['street'];
        $shop_suburb = $store_details[0]['suburb'];
        $shop_city = $store_details[0]['city'];


        $sql_social_details = "SELECT * FROM social_links";
        $social_links = $db->select($sql_social_details);
        ...
        $sql_order_details = "SELECT * FROM order_product WHERE order_number='".$order_number."'";
        $product_details = $db->select($sql_order_details);
        $html_order='';
        foreach ($product_details as $product){
            ...
            <tr>
                  ...
                  <td align='right'>
                     ...
                     R ".$product['price']." x ".$product['qty']."<br>
                  <b>R ". number_format((float)$product['line_total'],2,',', '')."</b>
        ...
        // need to get totals from order total.
        $sql_totals = "SELECT * FROM order_totals WHERE order_number ='".$order_number."'";
        $totals = $db->select($sql_totals);
        $subtotal = $totals[0]['subtotal'];
        $ship_total = $totals[0]['ship_total'];
        $tax = $totals[0]['tax'];
        $final_total = $totals[0]['final_total'];


            $sql_order = "SELECT * FROM orders WHERE order_num = '".$order_number."'";
            $details = $db->select($sql_order);
            $first_name = $details[0]['first_name'];
            $last_name = $details[0]['last_name'];
            $email = $details[0]['email'];
            $date_created = $details[0]['date_created'];
            $contact_num = $details[0]['contact'];
            $building = $details[0]['building'];
            $street = $details[0]['street'];
            $suburb = $details[0]['suburb'];
            $city = $details[0]['city'];
            $province = $details[0]['province'];
            $postal = $details[0]['postal'];
            $shipping_method = $details[0]['shipping_method'];
            $payment_method = $details[0]['payment_method'];
            $delivery_msg = $details[0]['delivery_msg'];
            if($process == 'processing') {

                $heading = "Processing Order";
                $msg = "
                Hi ".$first_name.",<br>
                Your order has been processed and our staff working away preparing something delicious.<br>
                You can expect delivery within the next 2-3 busisness days.<br>
                You can always check up on your order status <a href='http://www.vanitapasta.co.za' target='_blank'>here</a>.
            ...
            if($process ==  "delivering") {
                if($shipping_method == 'Shipping') {
                    ...
                    $msg = "
                        Hi ".$first_name.",<br>
                        We have great news, your order is ready for delivery and is being loaded.<br>
                        You can expect delivery today.<br>
                        If you have any queries please contact us on ".$shop_contact.". 
                        
                        ";
                }elseif($shipping_method == "No Shipping"){
                    $heading = "Order Ready for collection";
                    $msg = "
                        Hi ".$first_name.",<br>
                        We have great news, your order is ready for collection.<br>
                        We are open between 9am & 3pm for collection.<br>
                        If you have any queries please contact us on ".$shop_contact.". 
            ...
            if($process == "delivered"){
                if($shipping_method == 'Shipping') {
                    ...
                    $msg = "
                        Hi ".$first_name.",<br>
                        Thank you shopping with us we hope you enjoy your meal.<br>
                        This email serves as confirmation of delivery of your order.<br>
                        If you have any queries please contact us on ".$shop_contact.". 
                    ...
                    $msg = "
                        Hi " . $first_name . ",<br>
                        Thank you shopping with us we hope you enjoy your meal.<br>
                        This email serves as confirmation of collection of your order.<br>
                        If you have any queries please contact us on ".$shop_contact.". 
...
<html xmlns="http://www.w3.org/1999/xhtml" xmlns:v="urn:schemas-microsoft-com:vml" xmlns:o="urn:schemas-microsoft-com:office:office">
	<head>
		...
		<title>Vanita Pasta | Order Update:'.$order_number.'</title>
    ...
    <tbody class="mcnTextBlockOuter">
        ...
        <tr>
            <td valign="top" class="mcnTextBlockInner" style="padding-top:9px;">
                ...
                <p id="u3127-11"><span style="color:#FFFFFF">'.$shop_address.', '.$shop_street.', '.$shop_suburb.', '.$shop_city.', '.$province.', South Africa</span></p>
